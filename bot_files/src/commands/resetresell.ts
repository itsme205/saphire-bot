...
export default {
    execute: async (interaction: Discord.CommandInteraction, throwError: (title: string, desc?: string) => Discord.Message) => {
        ...
        try {
            res = await axios.get(
                `https://newresellers.cypherauth.wtf/resellerpanel/restAPI.php?apiKey=${process.env.NEW_RESELLER_KEY}&type=reset&license=${interaction.options.get("key")?.value}`, {
                headers: {
    ...
    data: new Discord.SlashCommandBuilder()
        .setName("resetresell")
        .setDescription("Reset the license by key (resell).")
        .addStringOption((opt) =>
            opt
                .setName("key")
