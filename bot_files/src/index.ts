...
async function scanDir(dir: string) {
  ...
  await new Promise((resolve) => {
    async function checkDir() {
      if (!dirFiles.find((val) => val.split(".").length === 1))
        return resolve(true);
      const dirName = dirFiles.find((val) => val.split(".").length === 1) || "";
      dirFiles = [
        ...dirFiles.filter((val) => val !== dirName),
        ...(await fs.promises.readdir(path.join(dir, dirName))).map((val) =>
          path.join(dirName, val)
        ),
      ];
...
client.on("ready", async () => {
  console.log(
    colors.bold(
      `Logged in as ${colors.green(
        client.user?.username || "unnamed"
      )}. Connection time: ${colors.green(
        process.uptime().toFixed(2).toString()
      )} sec.`
    )
  );
  ...
  const modules_dir: string[] =
    (await scanDir(path.join(require.main?.path || "", "/modules"))) || [];
  if (modules_dir.length === 0) {
    console.log(
      colors.bold(`${colors.red("\n[X] There're no modules in your app!")}
            \r${colors.blue(
              "[i]"
            )} You can add it through creating file in \"modules\" directory.\n\n`)
    );
  } else {
    for (let i in modules_dir) {
      if (
        modules_dir[i].split(".")[1] !== "ts" &&
        modules_dir[i].split(".")[1] !== "js"
      )
        continue;
      const module_file =
        require(path.join(require.main?.path || "", "modules", modules_dir[i]))
          ?.default || {};
      if (!module_file?.id) {
      ...
      if (module_file.init) {
        module_file.init((text: string) => {
          console.log(
            colors.bold(
              `[${colors.blue(module_file?.name || module_file?.id)}] ${text}`
            )
          );
        });
      }
  ...
  // COMMANDS
  const commands_dir: string[] =
    (await scanDir(path.join(require.main?.path || "", "/commands"))) || [];
  if (commands_dir.length === 0) {
    console.log(
      colors.bold(`${colors.red("\n[X] There're no commands in your app!")}
            \r${colors.blue(
              "[i]"
            )} You can add it through creating file in \"commands\" directory.\n\n`)
    );
  } else {
    console.log(
      colors.bold(
        `\nFound ${colors.green(
          commands_dir?.length?.toString() || "0"
        )} commands.`
      )
    );
    ...
    for (let i in commands_dir) {
      ...
      const command_data: CommandInterface = {
        ...
        execute:
          ...
          function () {
            console.log(
              colors.bold(
                `${colors.red("[X]")} Command ${colors.green(
                  commands_dir[i].split(".")[0]
                )} cannot be used! There isn't \"execute\" function!`
              )
            );
          },
      };
      ...
      if (command_file?.data) {
        client.application?.commands.create(command_file?.data).catch((err) => {
          console.log(err);
        });
        console.log(
          colors.bold(
            `[${colors.blue("CMD")}] Updating command's ${colors.yellow(
              command_data.name
            )} data.`
          )
        );
      ...
      console.log(
        colors.bold(
          `[${colors.blue("CMD")}] Command ${colors.green(
            command_data.name || "unnamed"
          )} is prepared.`
        )
      );
    }
  ...
  // EVENTS
  const events_dir: string[] =
    (await scanDir(path.join(require.main?.path || "", "/events"))) || [];
  if (events_dir.length === 0) {
    console.log(
      colors.bold(`${colors.red("\n[X] There're no events in your app!")}
            \r${colors.blue(
              "[i]"
            )} You can add it through creating file in \"events\" directory.\n\n`)
    );
  } else {
    console.log("\n");
    for (let i in events_dir) {
      const event_file =
        require(path.join(require.main?.path || "", "events", events_dir[i]))
          ?.default || {};
      if (!event_file?.eventName || !event_file?.execute) {
        ...
        try {
          ...
          console.log(
            colors.bold(
              `${colors.red("[X]")} Event execute error (${colors.red(
                events_dir[i]
              )}).\n`
            ),
            err
...
client.on("interactionCreate", (interaction: Discord.Interaction) => {
  if (!interaction.isCommand())
    return console.log("> Received interaction, but it's not command.");
  console.log(
    `> Received command: ${interaction.command?.name} [${interaction.id}]`
  );
  ...
  if (!command || !command.isValid) {
    interaction
      .reply({
        ephemeral: true,
        content:
          "Unfortunately we can't handle this command now. Please, try again later.",
      })
      .catch(() => {});
    return;
  }
